        ...
        items.map((row, i) => (
          <View style={{ padding: 8, borderColor: 'grey', borderBottomWidth: 1/PixelRatio.get()}} key={`his${i}`} >
            <TouchableOpacity onPress={() => { uistore.region.updatePoint(row.latitude, row.longitude); Actions.Main() }}>
              <Text style={{ fontSize:16, marginBottom: 4 }}><Text style={{color: '#ff8c0b'}}>${row.price}</Text>/L <Text style={{flex:1, textAlign: 'right', fontSize: 12}}> {row.date.format('MMMM DD, h:mm:ss a')}</Text></Text>
